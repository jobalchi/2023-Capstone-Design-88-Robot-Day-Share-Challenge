    ...
    def __init__(self):
        ...
        self.pose_1 = [
            ...
            [1.7092743308154927, 6.5968739818615845,
                0.7238265429552857, 0.6899819821657663],
            [2.058541494313217, 2.1171734664138664, 0.024931048294,-0.9996891731]]
        ...
        self.pose_2 = [
            ...
            [1.3591987613561451, 6.5968739818615845,
                0.7238265429552857, 0.6899819821657663],
            [0.3335821772954182, 6.0361350449018145, -0.6516767505260064, 0.7584968113471969]]
...
#Subscriber List
        ...
        self.aruco_sub = self.create_subscription(FiducialTransformArray,
            '/aruco_detect/fiducial_transforms',
    ...
    def aruco_detect_callback(self,aruco_data):
            ...
            try:   
                if aruco_data is not None:
                    self.arucoPose= aruco_data.transforms[0]
                    self.arucoDist = round(self.arucoPose.transform.translation.z,2)
                    self.arucoOrnt = round(self.arucoPose.transform.translation.x,2)
                    self.arucoFlag=True
            except:
                    print("no data")
                    self.arucoFlag=False
            ...
            if self.startCmd == 1:
                ...
                if self.navigator.isTaskComplete() and (self.navStart1==False or self.navStart2==False):
                    ...
                    if self.arucoFlag == True:
                        if self.arucoDist > 0.40 and self.arucoMove==True:
                            self.tracer_cmd.linear.x = +0.15
                            # self.cmd_pub.publish(self.tracer_cmd)
                            if self.arucoOrnt<0.0:
                                self.tracer_cmd.angular.z = +0.1
                            if self.arucoOrnt>0.0:
                                self.tracer_cmd.angular.z = -0.1
                            self.cmd_pub.publish(self.tracer_cmd)
                        elif self.arucoDist<0.40:
                            print("Pose Reached")
